                ...
                if (isset($_GET['item']) && isset($_GET['price'])) {
                    ...
                    if (isset($_POST['submit'])) {
                        ...
                        if ($amountToAdd > 0) {
                            ...
                            $updateMoneyQuery = "UPDATE guest_registration SET money=? WHERE name=?";
                            $stmt = mysqli_prepare($con, $updateMoneyQuery);
                            mysqli_stmt_bind_param($stmt, "ds", $newMoney, $name);
                    ...
                    if ($money >= $giftCost) {
                        ...
                        $updateItemQuery = "UPDATE items SET is_available = 0 WHERE id = ?";
                        $stmt = mysqli_prepare($con, $updateItemQuery);
